...
function getLatestCSVForSystem2() {
    $currentYear = date('Y');
    $previousYear = $currentYear;
    $pattern = sprintf('/(%s)-(?:1st|2nd|3rd|4th|5th)-1-2\.csv$/', $previousYear . '|' . $currentYear);
    $files = glob('*.csv');
...
$csv_file = getLatestCSVForSystem2();
$today = date('Y/m/d');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
    ...
    if (move_uploaded_file($uploadedFile, $imagePath)) {
        ...
        $redirectURL = "https://globalsteptest.com/katsurao/1-2.php?ph=$ph_result&do=$do_result&temperature=$temp2_result&salinity=$salt_result";
