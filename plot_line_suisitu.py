    ...
    try:
        ...
        for i, suisou_name_list in enumerate(suisou_name_lists):
            for suisou_name in suisou_name_list:
                ...
                if os.path.exists(csv_file_path):
                    ...
                    suisitu_df.set_index(suisitu_df.columns[1], inplace=True)
        ...
        for i, suisitu_df_list in enumerate(suisitu_df_lists):
            if any(not df.empty for df in suisitu_df_list):
                ...
                fig.suptitle(f"Line Plot for {target_type} (Kei: {i+1})")
                ...
                for j, suisitu_df in enumerate(suisitu_df_list):
                    if not suisitu_df.empty and target_type in suisitu_df.columns:
                        ...
                        if not tempdf.empty:
                            # 折れ線グラフを描画
                            axes[j].plot(tempdf.index, tempdf.values, marker='o', linestyle='-', color='b')
                            axes[j].set_title(f"{suisou_name_lists[i][j]}")
                            axes[j].set_xlabel('Day')
                            axes[j].set_ylabel(target_type)
                            
                            # target_typeに応じたy軸の範囲を設定
                            if target_type in y_range_mapping:
                                y_min, y_max = y_range_mapping[target_type]
                ...
                image_path = os.path.join(image_dir, f'line_plot_kei_{i+1}.png')
